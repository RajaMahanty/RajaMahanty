...
if (contactForm) {
	contactForm.addEventListener("submit", async (e) => {
		...
		const honeypotValue = formData.get("company_url");
		if (honeypotValue && honeypotValue.trim() !== "") {
			// Silently reject the submission - don't let bots know they failed
		...
		try {
			...
			const response = await fetch("https://formspree.io/f/mrboodwp", {
				method: "POST",
				body: formData,
				headers: {
...
// Gradients are now applied directly in HTML for better visibility
// This ensures project cards have vibrant backgrounds even without images
